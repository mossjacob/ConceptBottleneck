...
from CUB.template_model import MLP, inception_v3, End2EndModel, ConvDecoder, VAE
...
def ModelXtoCtoY(n_class_attr, pretrained, freeze, num_classes, use_aux, n_attributes, expand_dim,
    ...
    if n_class_attr == 3:
        ...
        attr_model = MLP(input_dim=n_attributes, num_classes=num_classes, expand_dim=expand_dim)
    encoder = model1
    decoder = ModelYtoX()
    return VAE(encoder, decoder, attr_model)


# Joint Model standard loss
def CBMModelXtoCtoY(n_class_attr, pretrained, freeze, num_classes, use_aux, n_attributes, expand_dim,
                 use_relu, use_sigmoid):
    model1 = inception_v3(pretrained=pretrained, freeze=freeze, num_classes=num_classes, aux_logits=use_aux,
                          n_attributes=n_attributes, bottleneck=True, expand_dim=expand_dim,
                          three_class=(n_class_attr == 3))
